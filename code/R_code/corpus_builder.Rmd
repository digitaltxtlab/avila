...
In the cleaning process, all footnotes are removed from the corpus - if you wish to include these (or filter out others), you can change the tm_filter line in the cleaning function  
Lastly, the corpus is then saved as both a .txt and an RData file, which can also be found in the "data" folder
...
for (i in 1:length(books.cor)){
  ...
  books.cor[[i]]$meta$sender_city <- as.character(meta$sender_city[[i]])
  books.cor[[i]]$meta$reciever <- as.character(meta$reciever[[i]])
...
clean_corpus <- function(corpus){
  ...
  tm_map(removeWords, stop.words) %>% #removes all words in stopword_us.txt. Alternatively use: stopwords("english")
...
clean.corpus <- clean_corpus(books.cor) #returns the clean corpus

save.corpus.to.files(clean.corpus, filename = "data/clean_corpus") #saves the cleaned corpus to a .txt and .RData file
...
writeLines(as.character(clean.corpus[[30]]))
